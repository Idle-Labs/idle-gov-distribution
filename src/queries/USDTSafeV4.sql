WITH idleV4_txs AS (
    ...
    WHERE
        idleV4."evt_tx_hash" IN (SELECT evt_Transfer."evt_tx_hash" FROM idle_v4."IdleUSDT_v4_Risk_evt_Transfer" evt_Transfer)
    AND
        idleV4."contract_address" IN ('\x28fAc5334C9f7262b3A3Fe707e250E01053e07b5')
    ORDER BY
...
idleV4_redeems AS (
    ...
    INNER JOIN
        idle_v4."IdleUSDT_v4_Risk_call_redeemIdleToken" redeemIdleToken
        ON redeemIdleToken."call_tx_hash"=idleV4_txs."evt_tx_hash"
    WHERE
        redeemIdleToken.call_success='true'
        AND idleV4_txs.from<>'\x0000000000000000000000000000000000000000'
),
idleV4_mints AS (
    SELECT
        idleV4_txs.*,
    ...
    INNER JOIN
        ...
        ON idleV4_txs."evt_tx_hash"=idleV4_txs_2."evt_tx_hash" AND idleV4_txs."evt_index"=idleV4_txs_2."evt_index"
    INNER JOIN
        idle_v4."IdleUSDT_v4_Risk_call_mintIdleToken" mintIdleToken
        ON mintIdleToken."call_tx_hash"=idleV4_txs."evt_tx_hash"
    WHERE
        mintIdleToken.call_success='true'
        AND idleV4_txs.to<>'\x0000000000000000000000000000000000000000'
),
